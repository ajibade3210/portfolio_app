...
export async function createProject(formData: FormData) {
  ...
  const design_flow = (formData.get("case_design_flow") as string)?.split("\n").filter(Boolean) || []
  const schema_design = formData.get("case_schema") as string || ""
  const tech_decisions = (formData.get("case_tech_decisions") as string)?.split("\n").filter(Boolean) || []
  const challenges = (formData.get("case_challenges") as string)?.split("\n").filter(Boolean) || []
  const learnings = (formData.get("case_learnings") as string)?.split("\n").filter(Boolean) || []
