...
export default async function ProjectCaseStudyPage({ params }: Props) {
  ...
  const hasCaseStudy = cs && Object.values(cs).some(v => 
      ...
      {!hasCaseStudy ? (
        <section className="rounded-lg border border-border bg-card p-8 text-center">
          <p className="text-muted-foreground">
            Detailed case study coming soon. Check back later for the full breakdown of this project.
          </p>
        </section>
      ) : (
        <div className="flex flex-col gap-12">
          ...
          {cs.overview && (
            <section>
              ...
              <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{cs.overview}</p>
            </section>
          )}
            ...
            <section className="grid gap-8 md:grid-cols-2">
              {cs.problem && (
                <div>
                  ...
                  <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{cs.problem}</p>
                </div>
              )}
              {cs.solution && (
                <div>
                  <h2 className="text-2xl font-semibold text-foreground mb-4">The Solution</h2>
                  <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{cs.solution}</p>
                </div>
              )}
            </section>
          )}
          ...
          {cs.design_flow && cs.design_flow.length > 0 && (
            <section>
              <h2 className="text-2xl font-semibold text-foreground mb-4">Design Flow</h2>
          ...
          {cs.schema_design && (
            <section>
              <h2 className="text-2xl font-semibold text-foreground mb-4">Schema Design</h2>
              <div className="rounded-lg border border-border bg-card p-6 overflow-x-auto">
                <pre className="text-sm text-muted-foreground font-mono whitespace-pre-wrap">{cs.schema_design}</pre>
              </div>
            </section>
          )}
          ...
          {cs.tech_decisions && cs.tech_decisions.length > 0 && (
            <section>
              <h2 className="text-2xl font-semibold text-foreground mb-4">Technical Decisions</h2>
          ...
          {cs.learnings && cs.learnings.length > 0 && (
            <section>
              <h2 className="text-2xl font-semibold text-foreground mb-4">Key Learnings</h2>
              <ul className="flex flex-col gap-3">
          ...
          {cs.results && (
            <section>
              ...
              <div className="rounded-lg border border-border bg-card p-6">
                <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{cs.results}</p>
              </div>
            </section>
          )}
        </div>
